...
public class Maintenance {
	...
	{		
		...
		try {			
			...
			//
			System.out.println("codeText--" + codeText.replace("/n", "--"));//----------------------------------------------------sysout
			char corrAnsw = 'l';
			System.out.println("correctAnswer--"+correctAnswer);
			if(correctAnswer != null && correctAnswer != "" && correctAnswer.length() > 0){
				corrAnsw = correctAnswer.charAt(0);
			}
			actionRes = maintenanceService.CreateNewQuestion( imageLink ,  questionText, 
					category,  Integer.parseInt(levelOfDifficulty), answers, corrAnsw ,
	...
	public String UpdateProcessingPage(String questionID, String questionText,
		...
		char corrAnsw = 'l';
		if(correctAnswer != null && correctAnswer.length() > 0 && correctAnswer != ""){
			corrAnsw = correctAnswer.charAt(0);
		}
		// 
		boolean result = maintenanceService.UpdateTextQuestionInDataBase( questionID,  imageLinkText,  questionText,  category,
				Integer.parseInt(levelOfDifficulty),  answers,  corrAnsw , codeText, numAnswersOnPictures);
		String outRes = "";
		if (result) {
	...
	public String CreationUpdateForm(String questionID, Model model) {	
		...
		try {
			if ((Integer.parseInt(questionID)) > 0) {				
				...
				if(questionElementsArray.length == 12){
...
@RequestMapping(value="/create-code-question",method=RequestMethod.POST)
public @ResponseBody JsonResponse CreateCodeQuestion(HttpServletRequest request) {   
String personCode = request.getParameter("personCode");
System.out.println(" create personCode-"+personCode);
...
String personCode = request.getParameter("personCode");
System.out.println("handler personCode-"+personCode);
JsonResponse res = new JsonResponse(); 
//long tRes = companyService.getCompanyByName(name_company);
if(tRes == -1){    			
	res.setStatus("SUCCESS");
